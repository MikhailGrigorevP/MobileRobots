...
int num = 0;
	...
	void AI::dfs_components(ED_N::environmentDescriptor* environment, vector<vector<unsigned>>& field, vector<managementComponent*> components, int c_n) {
	
		bool scanning;
		vector<Components_N::managementComponent*>::iterator component = components.begin() + c_n;
	...
	void AI::dfs(ED_N::environmentDescriptor* environment, vector<vector<unsigned>>& field, vector<Components_N::managementComponent*> components) {
		...
		while (_ai_) {
			...
			{
				if (components[i]->getActive() == true)
					_threads.push_back(thread(&AI::dfs_components, this,
						environment, std::ref(field), components, i));
			}
